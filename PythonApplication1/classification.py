...
def classify_sentences(text):
    ...
    for sentence in test:
        ...
        merged_sentence=newtext
        
        if any(keyword in merged_sentence.lower() for keyword in prio_keywords):
                        priority_found = True
                        priority_sentence=merged_sentence
                        both_sides_sentences.append(merged_sentence)
        elif priority_found == True and not merged_sentence.isspace():
                        merged_tokens=nlp(merged_sentence)
                        ...
                        if similarity >=0.5 or similarity2>=0.5 or similarity4>=0.5:
                            left_rightadd(priority_sentence,merged_sentence)
                        else:
                            priority_found= False
                            left_right(merged_sentence)      
        else: left_right(merged_sentence)       
